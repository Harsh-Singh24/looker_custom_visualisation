...
pivots.forEach(p=>{
const cell=row[measures[0].name][p.key]
flatRow.push({
cell:cell,
field:measures[0]
})
...
html+=`<th class="${cls}" data-index="${i}">
${h.label}
...
(totalsEnabled.length===0 || totalsEnabled.includes(c.field.name))
){
columnTotals[i]+=val
}
...
html+=`<td class="${numeric} cell-drill" data-row="${r}" data-col="${i}">
${formatValue(c.cell)}
</td>`
...
const field = flattenData().headers[colIndex]
const rowData = data[rowIndex]

let cellData

if(field.type==="dimension"){
cellData=rowData[field.name]
}else if(hasPivot){
cellData=rowData[field.name][field.pivot]
}else{
cellData=rowData[field.name]
}
