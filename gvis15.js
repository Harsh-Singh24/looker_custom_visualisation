  ...
  updateAsync: function(data, element, config, queryResponse, details, done) {
    ...
    function render() {
      ...
      headers.forEach(function(h, i) {
        ...
        // --- FIX 3: RESTORE PURE USER LABELS FOR TABLE CALCS ---
        let fieldName = h.label;
        if (h.field) {
          fieldName = h.field.label_short || h.field.label || h.field.name;
        }

        // Apply input customization if available
        if (customLabel && customLabel.trim() !== "") {
          fieldName = customLabel;
        }

        // --- FIX 1: POPULATE MULTI-LAYER PIVOT GRID HEADERS ---
        if (h.pivot) {
          var pivotObj = pivots.filter(function(p) { return p.key === h.pivot; })[0];
          var pivotLabel = (pivotObj && pivotObj.labels) ? pivotObj.labels.join(' - ') : h.pivot;
          fieldName = pivotLabel + " - " + fieldName;
        }

        // --- FIX 2: SEPARATE EXPLORE LABELS FROM METRIC WEIGHTS ---
        let headerText = "";
