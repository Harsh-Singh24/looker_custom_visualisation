...
if(m.is_table_calculation){

if(m.can_pivot){
pivotFields.push(m)
}else{
nonPivotCalcs.push(m)
}
...
headers.push({
type:m.is_table_calculation?"calc":"measure",
name:m.name,
label:m.label,
field:m
})
...
let numeric=headers[i].type==="dimension"?"":"numeric"
let calcClass=headers[i].type==="calc"?"calc-cell":""

html+=`
<td class="${numeric} ${calcClass} cell-drill"
data-row="${r}"
data-col="${i}">
${formatValue(c.cell)}
</td>
